...
_VALID_REF = "secrets:mnemoseed/dream/deep_reflection"
...
def _config_toml(tmp_path: Path, *, role_table: str = "") -> Path:
    ...
    path.write_text(
        ...
        'driver = "stub"\n'
        'model = "stub"\n' + role_table + "[dream.llm.short_increment]\n"
        'driver = "stub"\n'
        'model = "stub"\n',
        encoding="utf-8",
    )
...
def test_is_secrets_ref_detects_the_prefix() -> None:
    assert is_secrets_ref("secrets:mnemoseed/dream/deep_reflection") is True
...
def test_secret_name_from_ref_extracts_the_store_name() -> None:
    assert secret_name_from_ref(_VALID_REF) == "mnemoseed/dream/deep_reflection"
...
def test_write_accepts_env_name_list_unchanged(tmp_path) -> None:
    ...
    service.set(
        "dream.llm.deep_reflection.api_key_env",
        "MNEMOSEED_DEEP_REFLECTION_API_KEY,FIREWORKS_API_KEY",
        actor="console",
    )
    assert 'api_key_env = "MNEMOSEED_DEEP_REFLECTION_API_KEY,FIREWORKS_API_KEY"' in path.read_text(
        encoding="utf-8"
    )
...
def test_write_accepts_a_valid_secrets_reference(tmp_path) -> None:
    service, path = _service(tmp_path)
    service.set("dream.llm.deep_reflection.api_key_env", _VALID_REF, actor="console")
    assert f'api_key_env = "{_VALID_REF}"' in path.read_text(encoding="utf-8")
    assert load_config(path).llm["deep_reflection"].params["api_key_env"] == _VALID_REF
...
def test_write_rejects_literal_key_like_values_without_echoing(tmp_path) -> None:
    ...
    for bad in ("sk-abc123", "sk-proj-deadbeef", "openai_api_key"):
        with pytest.raises(ConfigWriteError, match=r"config\[dream\.llm\.deep_reflection\.api_key_env\]"):
            service.set("dream.llm.deep_reflection.api_key_env", bad, actor="console")
...
def test_write_rejects_malformed_references(tmp_path) -> None:
    ...
    for bad in (
        ...
        "secrets:other/dream/deep_reflection",
        "secrets:mnemoseed/dream/deep_reflection,EXTRA",
    ):
        with pytest.raises(ConfigWriteError, match=r"config\[dream\.llm\.deep_reflection\.api_key_env\]"):
            service.set("dream.llm.deep_reflection.api_key_env", bad, actor="console")
...
def test_load_accepts_a_valid_reference(tmp_path, monkeypatch) -> None:
    ...
    path = _config_toml(
        tmp_path,
        role_table='api_key_env = "secrets:mnemoseed/dream/deep_reflection"\n',
    )
    assert load_config(path).llm["deep_reflection"].params["api_key_env"] == _VALID_REF
...
def test_load_rejects_a_malformed_reference_naming_the_key(tmp_path, monkeypatch) -> None:
    ...
    with pytest.raises(ConfigError, match=r"config\[dream\.llm\.deep_reflection\.api_key_env\]"):
        load_config(path)
...
def test_load_tolerates_a_hand_edited_literal_key(tmp_path, monkeypatch) -> None:
    ...
    assert cfg.llm["deep_reflection"].params["api_key_env"] == "sk-proj-literal-value"
...
def test_read_redacts_a_literal_key_but_keeps_references(tmp_path) -> None:
    from mnemoseed.configwrite.service import _redact_env_names
