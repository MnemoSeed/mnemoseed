...
from mnemoseed.secrets import ChainSecretStore, FileSecretStore, KeyringSecretStore
from mnemoseed.secrets.store import _probe_keyring_store
...
def _chain(tmp_path: Path, head: object | None, monkeypatch: pytest.MonkeyPatch) -> ChainSecretStore:
    if head is None:
        monkeypatch.setattr("mnemoseed.secrets.store._probe_keyring_store", lambda: None)
    else:
        monkeypatch.setattr("mnemoseed.secrets.store._probe_keyring_store", lambda: head)
