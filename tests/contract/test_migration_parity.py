...
def test_version_sequences_identical_on_both_dialects() -> None:
    """The dialect-agnostic sequence IS the parity baseline (graph 1,2,5; meta 1,3,4,6,7)."""
    ...
    assert meta_versions == [1, 3, 4, 6, 7]
...
def test_sqlite_v1_to_head_forward_migration_preserves_data() -> None:
    ...
    # forward migration: graph advances to 5, meta to 7 (v2/v5 are graph-only,
    # v6 is meta-only: identity users table + hashed token column; v7 is the
    # profile archive flag)
    assert apply_migrations(graph, "graph") == 5
    assert apply_migrations(meta, "meta") == 7
    assert current_schema_version(graph, "graph") == 5
    assert current_schema_version(meta, "meta") == 7
    ...
    for table in _META_TABLES:
        # v6 adds the nullable token_hash column and v7 the profiles.archived
        # flag; legacy rows are preserved byte-for-byte, projected without the
        # new (empty/zero) columns.
    ...
    assert sorted(meta_versions) == [1, 3, 4, 6, 7]
...
def test_postgres_v1_to_head_forward_migration_preserves_data() -> None:
    ...
    with conn.transaction():
        ...
        assert apply_postgres_migrations(conn, "meta", schema=meta_schema) == 7
        assert current_postgres_schema_version(conn, "graph", schema=graph_schema) == 5
        assert current_postgres_schema_version(conn, "meta", schema=meta_schema) == 7
