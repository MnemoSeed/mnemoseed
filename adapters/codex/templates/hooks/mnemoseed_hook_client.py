"""Shared plumbing for the MnemoSeed Codex CLI adapter.
...
design: hook subprocesses run inside arbitrary Codex CLI environments and must
not depend on the mnemoseed package or a prepared virtualenv.

Fail-open contract (FR-6.3c): a hook always exits 0, never blocks the agent, and
...
Codex read paths (design/06 section 2.5): the SessionStart warm-up and the
UserPromptSubmit per-turn injection (`additionalContext`, capped at 2,500
tokens by the host) carry reads into the session; AGENTS.md provides standing
guidance. SessionEnd settles by reading the session transcript
(``transcript_path`` rollout.jsonl) for full-fidelity capture first.
"""
...
# Codex caps per-turn `additionalContext` at 2,500 tokens (design/06 2.1).
DEFAULT_TURN_BUDGET_TOKENS = 2500
COMMAND_BUDGET_TOKENS = 800
DEFAULT_TOOL_OUTPUT_CAP = 100_000
# Cap the SessionEnd transcript read to the tail of the file so the final
# session record stays bounded and the settle never reads an unbounded file.
DEFAULT_TRANSCRIPT_CAP = 200_000
...
def read_transcript(path: str) -> str:
    """Best-effort read of the Codex rollout.jsonl transcript.

    Returns at most the last ``DEFAULT_TRANSCRIPT_CAP`` characters so the final
    session record is bounded. A missing, unreadable, or empty file returns an
    empty string; nothing here ever raises (fail-open).
    """
    try:
        size = os.path.getsize(path)
        if size <= 0:
            return ""
        read_len = min(size, DEFAULT_TRANSCRIPT_CAP)
...
def handle_session_start(data: dict[str, Any], budget: HookBudget) -> dict[str, Any]:
    """SessionStart warm-up (FR-6.3c): fetch context and inject ≤800 tokens."""
    cwd = data.get("cwd")
...
def handle_user_prompt_submit(data: dict[str, Any], budget: HookBudget) -> dict[str, Any]:
    """UserPromptSubmit (FR-6.3c): capture the prompt then inject a per-turn
    recall within Codex's ≤2,500-token `additionalContext` cap. Capture and
    injection share the one hook deadline and fail open."""
    ...
    # deterministic capture first, then injection
...
def handle_session_end(data: dict[str, Any], budget: HookBudget) -> dict[str, Any]:
    """SessionEnd (FR-6.3c): read the session transcript for full-fidelity
    capture, then settle the session (drain + end) so buffered turns run the
    F2-F4 funnel. Both steps fail open: a missing transcript still settles."""
