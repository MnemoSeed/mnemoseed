#!/usr/bin/env bash
# Resolve a working Python interpreter and exec the given hook script.
#
# Hook subprocesses run through the shell with an arbitrary environment. Codex
...
# The enclosing host shell passes a double-quoted `~` through unexpanded (a
# quoted tilde is not a tilde-prefix), so resolve a leading `~/` against the
# home directory ourselves; without this every hook would silently fail to
# find its script.
case "$SCRIPT" in
  "~/"*)
    if [ -n "${HOME:-}" ]; then
