"""Shared plumbing for the MnemoSeed Cursor adapter.
...
design: hook subprocesses run inside arbitrary Cursor environments and must not
depend on the mnemoseed package or a prepared virtualenv.

Fail-open contract (FR-6.3b): a hook always exits 0, never blocks the agent, and
...
Cursor read paths (design/06 section 2.5): per-turn injection is not available,
so reads rely on the session-start warm-up (``additional_context``), the
``.cursor/rules/*.mdc`` standing guidance, and the model's own ``memory.recall``
MCP calls.
"""
...
# Fallback session-start warm-up query. Profiles whose memory language differs
...
def read_stdin_json() -> dict[str, Any]:
    """The Cursor hook input object, read from stdin.
...
def _params(data: dict[str, Any]) -> dict[str, Any]:
    """Cursor nests hook event arguments under ``params``; other fields stay at
    the top level. Reading both defensively keeps the scripts shape-tolerant."""
...
def handle_session_start(data: dict[str, Any], budget: HookBudget) -> dict[str, Any]:
    """Cursor sessionStart warm-up (FR-6.3b): fetch context and hand it back as
    ``additional_context`` (fire-and-forget). Cursor offers no per-turn prompt
    injection, so this is the only automatic read path besides the rules file
    and the model's own ``memory.recall`` calls."""
...
def handle_post_tool_use(data: dict[str, Any], budget: HookBudget) -> dict[str, Any]:
    ...
    p = _params(data)
    tool_name = _first_str(p, "toolName") or _first_str(data, "tool_name")
    if not tool_name:
        return {}
    input_value = _pick(p, "toolInput", "tool_input") or _pick(data, "tool_input")
    output_value = _pick(p, "toolResponse", "tool_response") or _pick(data, "tool_response")
    content = {
        "tool_name": tool_name,
...
def handle_after_agent_response(data: dict[str, Any], budget: HookBudget) -> dict[str, Any]:
    """afterAgentResponse: capture the full assistant response text (FR-6.3b)."""
    p = _params(data)
    text = _first_str(p, "response", "transcript") or _first_str(data, "response", "transcript")
    if not text.strip():
        return {}
    post_ingest(budget, "assistant_message", _session_id(data), resolve_profile_id(), {"text": text})
    return {}
