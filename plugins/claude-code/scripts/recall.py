...
Prints a terse, budgeted context block on stdout (the command's stdout is
captured into the prompt by Claude Code). Never raises; a quiet failure reports
the daemon as unreachable and returns 0 so the agent can continue.
"""
