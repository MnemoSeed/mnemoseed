...
_PG_KINDS: dict[str, str] = {
    "TEXT": "TEXT",
    "INTEGER": "INTEGER",
    "REAL": "DOUBLE PRECISION",  # sqlite REAL is IEEE-754 double; keep exact values
    "JSON": "JSONB",
}


def _postgres_column_sql(column: Column) -> str:
    parts = [column.name, _PG_KINDS[column.kind]]
    if column.primary_key:
        parts.append("PRIMARY KEY")
        if column.kind == "INTEGER":
            # Postgres has no implicit rowid: integer-keyed tables (audit_log,
            # score_pool) self-generate the key like SQLite's rowid alias. BY
            # DEFAULT keeps the drivers' explicit id=1 singleton inserts legal.
            parts.append("GENERATED BY DEFAULT AS IDENTITY")
    if column.not_null:
        parts.append("NOT NULL")
    if column.default is not None and not (column.primary_key and column.kind == "INTEGER"):
        if isinstance(column.default, str):
            parts.append(f"DEFAULT {column.default!r}")
        else:
            parts.append(f"DEFAULT {column.default}")
...
def render_postgres(op: DDLOp) -> str:
    """Render one structured DDL operation as Postgres SQL.

    Consumes exactly the same ops as ``render_sqlite`` (AC-5 parity): a dialect
    render shared op definitions is a zero-difference archive of the frozen
    schema, only the SQL changes.
    """
    if isinstance(op, CreateTable):
        parts = [_postgres_column_sql(c) for c in op.columns]
        parts.extend(f"UNIQUE ({', '.join(cols)})" for cols in op.unique)
        body = ", ".join(parts)
        return f"CREATE TABLE IF NOT EXISTS {op.name} ({body})"
    if isinstance(op, CreateIndex):
        ...
        return f"CREATE {unique}INDEX IF NOT EXISTS {op.name} ON {op.table} ({cols})"
    if isinstance(op, AddColumn):
        # IF NOT EXISTS keeps re-runs idempotent (AC-5 upgrade on live data)
        return f"ALTER TABLE {op.table} ADD COLUMN IF NOT EXISTS {_postgres_column_sql(op.column)}"
    if isinstance(op, AddTrigger):
        if not op.pg_action:
            raise ValueError(f"AddTrigger {op.name!r} needs a pg_action for the Postgres dialect")
        return (
            f"CREATE OR REPLACE FUNCTION {op.name}() RETURNS trigger LANGUAGE plpgsql AS $$ "
            f"BEGIN {op.pg_action}; END $$;\n"
            f"DROP TRIGGER IF EXISTS {op.name} ON {op.table};\n"
            f"CREATE TRIGGER {op.name} {op.timing} {op.event} ON {op.table} "
            f"FOR EACH ROW EXECUTE FUNCTION {op.name}();"
        )
    raise TypeError(f"unknown DDL op {op!r}")
...
def ensure_postgres_schema_table(conn: Any) -> None:
    conn.execute(
        f"CREATE TABLE IF NOT EXISTS {SCHEMA_VERSION_TABLE} "
        "(version INTEGER PRIMARY KEY, applied_at TEXT NOT NULL, description TEXT NOT NULL)"
    )
...
def _applied_versions_pg(conn: Any) -> set[int]:
    rows = conn.execute(f"SELECT version FROM {SCHEMA_VERSION_TABLE}")
    return {int(row["version"]) if isinstance(row, dict) else int(row[0]) for row in rows.fetchall()}


def _ensure_pg_schema(conn: Any, schema: str) -> None:
    conn.execute(f'CREATE SCHEMA IF NOT EXISTS "{schema}"')
    conn.execute(f'SET search_path TO "{schema}"')


def apply_postgres_migrations(
    conn: Any, store: str, target: int | None = None, schema: str = "public"
) -> int:
    """Apply pending migrations to a Postgres connection in one transaction.

    Concurrent first-initialization runs serialize on a transaction-scoped
    advisory lock — the Postgres analog of SQLite's BEGIN IMMEDIATE. DDL is
    transactional in Postgres, so the whole migration batch either applies or
    rolls back. ``schema`` scopes the tables (D6 named instances).

    The schema/tracking-table setup runs inside the same transaction: psycopg3
    nests a ``transaction()`` block into a savepoint when the connection is
    already in a transaction, which would defer the COMMIT until an unrelated
    later statement. Starting the block on an idle connection (drivers call
    this at construction) makes it an outer BEGIN/COMMIT unit instead.
    """
    with conn.transaction():
        _ensure_pg_schema(conn, schema)
        ensure_postgres_schema_table(conn)
        conn.execute("SELECT pg_advisory_xact_lock(936982011)")
        applied = _applied_versions_pg(conn)
        for migration in _pending_for_store(store, target, applied):
            for op in migration.ops:
                if op.store == store:
                    conn.execute(render_postgres(op))
            conn.execute(
                f"INSERT INTO {SCHEMA_VERSION_TABLE} (version, applied_at, description) VALUES (%s, %s, %s)",
                (migration.version, iso8601_utc(time.time()), migration.description),
            )
    return current_postgres_schema_version(conn, store, schema=schema)


def current_postgres_schema_version(conn: Any, store: str, schema: str = "public") -> int:
    """Highest applied global version that touched ``store`` (0 when none)."""
    _ensure_pg_schema(conn, schema)
    ensure_postgres_schema_table(conn)
    applied = _applied_versions_pg(conn)
