...
writable key set live in :mod:`mnemoseed.config` (the established home for
...
from mnemoseed.config import DEFAULT_LAMBDA_PER_TYPE, LAMBDA_TARGETS
...
def lambda_for(node_type: str, lambda_per_type: Mapping[str, float]) -> float:
    """Resolve one type's λ: an explicit map entry wins, then the per-type
    design default, then the conservative fact rate."""
    if node_type in lambda_per_type:
        return float(lambda_per_type[node_type])
    return float(DEFAULT_LAMBDA_PER_TYPE.get(node_type, _FALLBACK_LAMBDA))
