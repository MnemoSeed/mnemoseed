...
flowchart TB
    ...
    Cmp -->|"有, 值不同"| Judge{"可自动裁决?<br/>(时间戳明确/来源权威度差 ≥ 阈值)"}
    Judge -->|"可裁决"| Invalidate["旧事实 kg_invalidate<br/>valid_to=now<br/>新事实接管"]
...
**强化回弹**：记忆每次被 Retrieve 命中并实际使用，`last_reinforced = now`，权重回到 `min(1.0, w + reinforcement_bonus)`——对应"间隔效应"（spacing effect）：被反复提取的记忆巩固加深。
