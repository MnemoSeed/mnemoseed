...
flowchart TB
    ...
    F3 -->|"S ≥ θ_capture"| Write["写入海马体 Chroma<br/>+ 完整元数据钢印"]
...
- **Verbatim 通道** = 海马体 Chroma 原始分片：有损操作（摘要/提炼）永远不允许发生在这里；
...
flowchart TB
    ...
    Cue --> Par["并发双路检索"]
    Par --> V["向量路：Chroma<br/>语义近邻 + 线索重叠加权<br/>× decay_weight"]
