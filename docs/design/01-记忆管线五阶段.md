...
**措辞不归引擎管，归 anima 演绎**——同一冲突对象，谨慎的 anima 会问得留有余地，直率的 anima 会单刀直入，像正常对话而不是系统弹窗（"你上周五还说要加前缀的，今天改主意了？要不要我把记忆彻底更新掉？"）。引擎保证事实准确，anima 演绎语气——说话方式是性格的函数，不是预写模板（见 [04 §2](04-隔离解耦与隐私.md) anima 模型）。
...
## 7. 偏好动力学：喜好 = anima 核心 × 经验

> 喜好不是静态字符串，是从人格底色长出、被事件塑形、随人生漂移的活体结构。
> 理论骨架与 anima 模型本体见 [04 §2](04-隔离解耦与隐私.md)；本节只定义偏好的更新机制。
...
- **Capture**：偏好不参与捕获评分（§1 红线）；但事件被捕获后作为证据进入偏好更新队列；
- **Consolidate**：梦境引擎批量应用偏好更新、重写 `idiographic_notes`、执行 anima 重染色（见 04 §2）；
