...
sequenceDiagram
    ...
    O->>D: ③ dream-model config wizard (post-setup step of onboard)<br/>① OAuth reuse of the host's local login state (Codex / Grok,<br/>both ToS-allowed; Anthropic subscriptions not reused; Chinese users may<br/>choose MiniMax/Kimi, with an explicit data-leaving-the-country notice)<br/>② bring your own API key (OpenAI-compatible endpoint, e.g. Fireworks)<br/>③ Ollama local model (quality note; both roles local = fully offline)<br/>only written to config.toml after a live connectivity test passes;<br/>skippable — skipping it leaves a bootable capture-only daemon (documented in the wizard)
...
2. **The LLM wizard is a POST-setup step** — it runs after the owner account exists, keeps its connectivity-test-before-persist behavior (config.toml written only after a live test passes), and is governed by the FR-6.9 order (OAuth reuse → BYOK → Ollama local model);
