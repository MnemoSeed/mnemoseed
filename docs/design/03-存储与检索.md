...
|---|---|---|---|
| `VectorStore` | 向量存取 + metadata 过滤 | Chroma 内嵌 | pgvector / Qdrant / Milvus / LanceDB |
| `GraphStore` | 图谱节点/边/版本链 | SQLite-Graph 或 Kuzu（内嵌零依赖） | Neo4j / Memgraph / Postgres+AGE |
| `MetaStore` | 配置、积分池、watermark、审计 | SQLite | Postgres |
| `Embedder` | 向量化 | embedding-gemma 本地 | 任意 OpenAI 兼容 embedding API |
