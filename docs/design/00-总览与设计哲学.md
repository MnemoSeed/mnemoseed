...
|---|---|---|
| **互补学习系统**（Complementary Learning Systems, McClelland 1995）：海马体快速记情节，新皮层慢速沉淀结构化知识 | 海马体 vs 大脑皮层分工 | **混合双库**：Chroma 向量池（海马体/热层）+ 知识图谱（皮层/冷层） |
...
| **情绪调制巩固**（杏仁核→海马体，去甲肾上腺素通路）：高情绪事件获得巩固优先权 | 闪光灯记忆 | 评分模型中**情绪强度**作为独立向量进入积分池 |
...
2. **多语言原生（Multilingual Native）**：统一 Embedding-Gemma 向量化，中英混排、代码混杂、方言语境下召回不掉精度。
...
flowchart TB
    ...
    subgraph Embed["Embedding-Gemma<br/>多语言向量化"]
    end

    Client -->|"MCP 协议"| MCPGW
    Intercept --> Stripper --> Scorer --> Pool
    Stripper -->|"gated write"| Chroma
    Pool -->|"触发"| Dream
    Chroma -->|"只读快照"| Dream
...
flowchart LR
    subgraph Local["STORAGE_MODE=local（免费/研发）"]
        L1["docker-compose:<br/>core + mcp + chroma<br/>+ embedding-gemma<br/>+ ollama(可选)"]
    end
    subgraph Cloud["STORAGE_MODE=cloud（付费 SaaS）"]
        C1["MnemoSeed Cloud:<br/>Neo4j Aura / 托管 Chroma<br/>AWS Nitro Enclaves 梦境<br/>E2EE 多端同步"]
    end
    Env[".env 一个变量"] --> Local
    Env --> Cloud
```
