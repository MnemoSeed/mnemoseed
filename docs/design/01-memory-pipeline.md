...
**Reinforcement rebound (with spacing-effect cooldown)**: every time a memory is hit by Retrieve and actually used, `last_reinforced = now`, and the weight rebounds to `min(1.0, w + 0.1)` (the rebound step is pinned at **0.1**, matching the capture-side Hebbian step so both event sides rebound identically) — but **repeated recall within a short time window yields diminishing returns** (rebound halves, then halves again, within the cooldown window): the spacing effect (Cepeda 2006 meta-analysis) proves that distributed review is far superior to massed review, and the same number of concentrated activations should not count as equivalent reinforcement. This also foils the speculation of "repeatedly pumping up one memory's weight". Additionally, near-duplicate hits at capture (§1 Hebbian encoding reinforcement) also trigger rebound (equally subject to the cooldown).
