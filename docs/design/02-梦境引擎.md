...
**规则**：
1. 读取侧检查是廉价的 Chroma metadata 过滤（要求钢印含 `entities` 字段，见 design/01 §1 与 PRD-01 FR-1.6 补充）；
2. 待巩固内容**降权 + 标注，绝不隐藏**——新旧并呈，选择权交给模型和用户；Persona 层渲染成自然语言（"我记得的是 X，不过最近有还没消化的新记录提到 Y"）；
