...
- **预算口径**：Delta 硬上限 10k tokens 是暂定值，随线上观测调整——若 `DeltaReport.overflow_count` 持续非零（增量积压、多次梦境清算不完），上调预算；overflow 积压量须经 memory.status / console 暴露（PRD-07），不做静默指标。
