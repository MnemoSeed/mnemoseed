...
1. **本地-云端对称（Local-Cloud Symmetry）**：DB、向量缓冲、梦境引擎全部抽象在接口后。改一个配置（`STORAGE_MODE` preset：embedded/docker/custom）即可在"纯本地离线（embedded 一键或 Docker）"与"自托管服务端"间切换；官方 SaaS 由我们托管 daemon（TEE 标配）。
...
5. **成本死锁**：任何一次发往云端高端模型的增量数据被工程机制死锁在动态预算硬上限内（≤32k tokens，按待清算积压量伸缩；月度 token 账本兜底总成本，详见 design/02 §6）。
...
flowchart TB
    ...
    subgraph MCPGW["mnemoseed-mcp（Python 瘦适配器，随 core 分发，stdio）"]
        Adapter["标准 MCP 协议适配<br/>转调 daemon localhost HTTP"]
    end
...
flowchart LR
    subgraph Local["STORAGE_MODE=local（免费/研发）"]
        L0["embedded 一键安装（默认）:<br/>单进程内嵌 LanceDB + SQLite-Graph<br/>+ bge-m3 ONNX，零 Docker 依赖"]
        L1["docker-compose（开发者/企业可选）:<br/>core（MCP server 为同包 stdio 入口，不占服务位）<br/>+ pgvector + pg_graph + pg_meta<br/>+ ollama(可选)"]
    end
    subgraph Cloud["STORAGE_MODE=cloud（云端 SaaS）"]
        C1["MnemoSeed Cloud:<br/>托管 daemon（SaaS，TEE 标配）<br/>E2EE 传输 + 加密存储<br/>ZDR 梦境出口"]
    end
    Env[".env 一个变量"] --> Local
    Env --> Cloud
```
...
- **MnemoSeed**：唯一把"五阶段管线 + 认知分级隔离 + 隐私优先托管"做成完整架构的中立层，且专精 **Agentic Tool-Use 肌肉记忆**（工具调用习惯序列的沉淀与继承）。
