...
1. **本地-云端对称（Local-Cloud Symmetry）**：DB、向量缓冲、梦境引擎全部抽象在接口后。改一个 `.env`（`STORAGE_MODE`）即可在"纯本地离线（embedded 一键或 Docker）"与"云端托管 SaaS/TEE"间零摩擦切换。
...
flowchart LR
    subgraph Local["STORAGE_MODE=local（免费/研发）"]
        ...
        L1["docker-compose（开发者/企业可选）:<br/>core（MCP server 为同包 stdio 入口，不占服务位）<br/>+ vector(lance) + embed(bge-m3)<br/>+ ollama(可选)"]
    end
    subgraph Cloud["STORAGE_MODE=cloud（云端 SaaS）"]
        C1["MnemoSeed Cloud:<br/>托管 daemon（SaaS）<br/>E2EE 传输 + 加密存储<br/>ZDR 梦境出口 / TEE 为可选档位"]
    end
    Env[".env 一个变量"] --> Local
    Env --> Cloud
```
