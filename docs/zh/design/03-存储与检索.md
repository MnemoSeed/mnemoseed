...
## 1. 存储层：Ports & Adapters 可插拔架构（2026-08-08 升级定稿）
...
**M0 范围纪律（2026-08-08 定稿）**：接口定好，每个接口先只实现**内嵌默认 + Postgres 系两个驱动**——第二个驱动用于实证接口可移植性，防止接口被第一实现绑架；其余驱动开放社区贡献。

**云端拓扑不变**：云端托管时各层指向托管集群 / 托管 PG（pgvector+pg_graph）/ embedding API / TEE 内梦境路由（`STORAGE_MODE` 当前有效值 = embedded/docker/custom，云端 preset 预留）。
...
sequenceDiagram
    ...
    HB->>RR: 融合重排
    Note over RR: score = α·语义相似度<br/>+ β·线索重叠度<br/>+ γ·decay_weight<br/>+ δ·图谱中心性<br/>+ ε·共现边权<br/>+ ζ·confidence（来源权威度）
...
**D1 已交付（2026-08-15，衰减，PRD-04）**：上表 consolidated 分片 λ × 3 一行已实现（`decay.model.lambda_for`，标记由梦境引擎写回）。`ChunkStamp.last_reinforced` 现已浮现在向量读路径上（`list_chunks` / `get_chunk`），衰减清扫的 chunk 遍只用既有端口读取强化基线——无需新增端口方法。

---

## 5. Docker Compose 生态（issue #5：按目的拆分）

```yaml
# 示意 —— 完整文件见 mnemoseed/core 仓库根目录 docker-compose.yml
# 默认（docker compose up）= 单容器跑 embedded 栈（与本地体验零差别）
services:
  core:              # Python/FastAPI 核心引擎；MCP server 为同包 stdio 入口，由宿主按需拉起，不占 compose 服务位
                     # MNEMOSEED_HOME=/data（持久卷），空卷默认 embedded preset

# 开发者/企业 PG 全家桶（docker compose --profile pg up）：
#   vector（pgvector）+ pg（postgres，承载 cortex graph + meta）+ embed（dev 嵌入 sidecar）
# 可选离线梦境模型：docker compose --profile ollama up
# 云端/VPS 部署：docker compose -f docker-compose.cloud.yml up -d（反代终结 TLS，见 design/10）
```
