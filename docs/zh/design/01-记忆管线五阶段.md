...
**强化回弹（含间隔效应冷却）**：记忆每次被 Retrieve 命中并实际使用，`last_reinforced = now`，权重回到 `min(1.0, w + 0.1)`（回弹步长钉死为 **0.1**，与捕获侧赫布步长一致，两个事件侧回弹语义相同）——但**短时间窗内的重复召回收益递减**（冷却窗口内回弹减半再减半）：spacing effect（Cepeda 2006 元分析）证明分散复习远优于扎堆复习，同样次数的集中激活不应等价强化。这同时防住"某记忆被反复刷权重"的投机。此外，捕获时的近重复命中（§1 赫布编码强化）同样触发回弹（同受冷却约束）。
