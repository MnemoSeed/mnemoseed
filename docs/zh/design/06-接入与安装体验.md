...
```bash
uv tool install mnemoseed && mnemoseed install   # pipx install mnemoseed 亦可
```
...
sequenceDiagram
    ...
    I->>D: ⑤ 梦境模型配置向导<br/>① OAuth 复用订阅（Codex / Grok 本地登录态，条款允许；<br/>Anthropic 订阅不复用；中国用户<br/>可选 MiniMax/Kimi，明示数据出境提示）<br/>② 自带 API key（OpenAI 兼容端点，如 Fireworks）<br/>③ 高级离线轨 Ollama（≤14B，质量警告）<br/>连通性实测通过才写入 config.toml
