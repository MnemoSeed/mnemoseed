...
上线 MnemoSeed Cloud：3 Profile 物理隔离、多端 E2EE 同步、Nitro Enclaves 内做梦、动态模型路由网关。云端全链路零明文持久化。

## 2. 范围

- **In**：E2EE 同步协议、多 Profile 隔离、Enclave 梦境执行环境 + attestation、动态路由网关（Sonnet / GPT-5.6 Terra）、用量计费
...
| FR-5.10 | Admin Plane 超管界面：服务健康/增长运营（注册/漏斗/license 激活）/用户运营（配额/封禁）/成本观测（模型路由分解/TEE 利用率）；**红线：仅运营元数据，记忆明文物理不可见（BYOK 架构保证，非纪律）**；超管独立强认证（TOTP/硬件密钥），全部操作进不可变审计日志 | P0 |
...
| NFR-5.2 | 单台 TEE 成本与用户容量配比须支持正毛利（具体数值目标为商业决策，见内部市场文档） |
