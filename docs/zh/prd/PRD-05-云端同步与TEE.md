...
| FR-5.10 | Admin Plane 超管界面：服务健康/增长运营（注册/漏斗/license 激活）/用户运营（配额/封禁）/成本观测（模型路由分解/资源利用率）；**模型路由配置位于 Admin Plane 层（系统级，作用于所有用户）**；**红线：仅运营元数据；TEE 档下记忆明文物理不可见（硬件保证），非 TEE 档靠加密存储 + 最小化运行时暴露面**；超管独立强认证（TOTP/硬件密钥），全部操作进不可变审计日志 | P0 |
