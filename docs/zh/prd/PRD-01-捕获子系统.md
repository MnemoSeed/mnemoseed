...
| FR-1.2 | Local Stripper 规则引擎：剥离编译日志、包管理输出、死循环报错堆栈、ANSI 高亮符、宿主注入系统工件（session 压缩摘要外壳、`<task-notification>` 转发块——非用户语音的任何结构性脚手架）；规则集可热更新 | P0 |
| FR-1.3 | 持久性分类（三个月测试）v1：规则（时间限定词/情绪词表）+ 词表与嵌入启发式（durable/disposable）；durable 标记家族覆盖偏好/决策/习惯规则/立场/开放性疑虑与设计探讨句（决策确认、开放疑问等），评分前先过 F1 剥离、全剥离 turn 判 disposable；**no-overfit 边界**：即时任务操作句保持 disposable——"这个 bug 怎么解决/接下来要开始部署"等一次性指令与排障询问不因决策/开放疑问词面翻转为 durable，仅有明确模态的里程碑放行（"接下去可以开始…"）与设计/产品类开放疑问（怎么看/如何确保、非代码对象的探讨）才算 durable；**v1 不引入边缘小模型**——先在基准标注集上实测 precision，未达 NFR-1.3 再以小模型补强（选型与校准届时以标注集为据） | P0 |
...
| NFR-1.2 | **可剥离噪音剥离率 ≥ 90%**（基准：真实 Claude Code session 日志；分子分母都只计规则命中的噪音类内容——编译日志/包管理输出/进度条/ANSI/宿主注入工件等）。全量字节压缩率降为观察指标（强依赖输入内容群体：编码重型日志 ≈90%+，研究/设计型对话实测仅 0.4%，不是缺陷），回归脚本两者都报 |
