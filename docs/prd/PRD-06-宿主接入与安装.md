...
> 里程碑：M1 · 预估 9.5 天（2026-08-08 宿主实测调研后修订）
...
| FR-6.2 | daemon embedded 模式：单进程内嵌 LanceDB + SQLite-Graph + bge-m3 ONNX（~150MB 模型下载含断点续传），零 Docker 依赖（默认栈选型见 design/03 §1） | P0 |
