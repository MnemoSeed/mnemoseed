...
| FR-6.9 | First-time LLM setup wizard (within the setup flow): guides dream-model configuration in the recommended order ① OAuth (reuse a subscription: Codex/ChatGPT; Chinese users may choose CLI providers such as MiniMax/Kimi, with an explicit data-residency-exit notice shown when selected) ② bring-your-own API key (any OpenAI-compatible endpoint, e.g. Fireworks) ③ the advanced offline track (Ollama, ≤14B quantized model, with a distillation-quality warning); config.toml is written only after connectivity passes; implements the PRD-02 FR-2.14 role routing | P0 |
