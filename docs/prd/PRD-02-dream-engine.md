...
Implement asynchronous "dreaming" consolidation: score-pool trigger → read-only snapshot → reflective distillation → dual-track split write-back → safe clear. Zero user-perceived latency and zero lost text throughout; dream models by default use OAuth / bring-your-own API key (zero hardware barrier); fully-offline operation is achieved by pointing both roles at Ollama — not a separate track (the quality warning remains).
...
| FR-2.7 | Fully-offline operation (**achieved by pointing both roles at Ollama**; no separate third track): Ollama + ≤14B quantized model runs the whole flow offline; first-time setup shows an explicit "distillation quality is lower than cloud large models" warning; 70B-class local models are not assumed by default | P1 |
...
| FR-2.15 | **Promotion quality gate** (design/02 §11): before write-back, every distilled triple passes three deterministic checks (groundedness entity-source overlap / conflict / near-duplicate reinforcement); failures get `promotion_status=quarantined` (excluded from retrieval); post-promotion, rolling promotion precision (user correction as first-class negative feedback) auto-tightens the gate when it drops (one-way ratchet); during the M1 manual phase the gate runs in shadow mode (advisory only), and auto-promotion unlocks only after its agreement rate with human review passes; rollback is reverse-versioning only | P1 |
| FR-2.14 | **LLM port and model-routing configuration**: define a `DreamLLM` Protocol (chat completion + usage accounting + connectivity self-check), with a driver registry isomorphic to the storage layer — drivers: `oauth` (reuses the host's existing login state: **Codex** (`~/.codex/auth.json`) and **Grok** (`~/.grok/auth.json`), both with OIDC auto-refresh and both allowed by their ToS; **Anthropic subscriptions are explicitly not reused** — `anthropic` is API-key-only; Chinese CLI providers such as MiniMax/Kimi optional with an explicit data-residency-exit notice) / `openai_compatible` (bring-your-own-key endpoints such as Fireworks) / `anthropic` (API key only) / `ollama` (local models, **non-default**); default recommended order OAuth > API key > offline; config.toml configures two roles: `deep_reflection` (long-context deep-sleep reflection) / `short_increment` (short increments, dynamic budget ≤32k, FR-2.5); the `local_track` config key is **deprecated** — accepted with a warning, ignored, no engine consumer, removed in a later version; default routing = the two per-role defaults: deep sleep → Kimi K3 (Fireworks), short increments → DeepSeek V4 Flash 0731 (Fireworks), with fully-offline operation achieved by pointing both roles at Ollama (FR-2.7); **keys are separated per role** — each role has its own default environment variable (`MNEMOSEED_DEEP_REFLECTION_API_KEY` / `MNEMOSEED_SHORT_INCREMENT_API_KEY`), falling back to the shared `FIREWORKS_API_KEY` when unset, allowing the two roles to attach different providers; each role can independently switch driver and model name, and changes are written to audit; the connectivity self-check interface is called by the console test button (design/07 §8) | P0 |
...
| NFR-2.4 | Local Ollama (≤14B quantized model, ordinary dev machine): single consolidation < 10 minutes |
