...
| FR-7.10 | Anima 面板（进阶模块，不在 M1 首发）：特质雷达图（轴数随 schema 不锁死六轴；顶点=mean，误差带=width 不确定性可视，允许手动微调）；白话创建（自然语言描述 → 模型量化生成模板）；核心实线 + 染层当前表现虚线叠加；跨 profile 链接/换绑入口 + 换绑触发 re-dye 确认；drift_history 时间轴回放（design/09 §7） | 进阶模块 |
