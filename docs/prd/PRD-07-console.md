...
| FR-7.11 | Every console write and settings change is backed by the **daemon-owned ConfigWriteService** (single config writer): registry → validate → surgical toml patch → versioned meta-store record (the existing `set_config`/`rollback_config` ports) → audit with actor attribution (console\|cli\|mcp) → live-apply or restart-required flag; config.toml is a generated mirror — the meta store is truth for registry keys (one-shot audited `config_import` on upgrade when the store is empty); a hand-edited file is detected by mtime/hash and the DB wins: the mirror is regenerated and a `config_mirror_drift` warning + audit entry is recorded (no more `config_rebaseline`); **configuration permissions are system-scoped**: ⑧/⑨ writes are owner/admin-level only (self-hosted = the owner account; commercial multi-user license = admin level, applies to all users; SaaS = the cloud Admin Plane, applies to all users) — never a per-user setting | P0 |
...
| G-AC2 | ⑧ configures all two dream roles (deep_reflection / short_increment); connectivity test passes before persist; the UI exposes env-var NAMES only (a literal key shown in the UI is a test failure); versioned + rollbackable |
...
1. `core/configwrite` — daemon-owned single config writer: registry → validate → surgical toml patch → versioned meta-store record (`set_config`/`rollback_config` ports) → audit (actor ∈ console|cli|mcp) → live-apply/restart-required flag; DB-primary for registry keys w/ one-shot `config_import`; mtime/hash drift detection → mirror regeneration + `config_mirror_drift` audit (2d)
...
4. `console/settings` + `console/models` — ⑨ Settings (w₁/w₂/w₃, λ per type, top-k, token budget, pool threshold; per-key live-vs-restart table; driver switch disabled with data) + ⑧ Models & Routing (two roles, env-var names only, connectivity test before persist, versioned + rollbackable) (1.5d)
...
10. GraphStore port extension: `list_edges(filter, page)` + the `GRAPH_EDGE_LIST` capability (value `graph.edge_list`; per PRD-08 Appendix B amendment v1.1) (1d)
...
- PRD-08 Appendix B amendment v1.1 ([PRD-08](../prd/PRD-08-m0-foundation.md) GraphStore `list_edges(filter, page)` + the `GRAPH_EDGE_LIST` capability, value `graph.edge_list`) — W3 prerequisite